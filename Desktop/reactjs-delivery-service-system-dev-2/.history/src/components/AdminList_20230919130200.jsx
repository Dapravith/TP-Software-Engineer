            ...
            {accounts.length > 0 && <div className="admin-list-container">
                ...
                </div>
                {
                    accounts.map((data, index) => {
                        return (
                            <li key={data._id} style={{background: adminEmail === data.email && "#3a913a", color: adminEmail === data.email ? "#ffffff" : "#000000"}}>
                                <div>
                                    <span>{index + 1}</span> {/* Displaying the serial number */}
                                    <div className="admin-pro" style={{background: `url(${process.env.REACT_APP_BACKEND}/admin/profile/${data._id}) no-repeat`}}></div>
                                    {data.email}
                                    {adminEmail === data.email && <div className="is-me">You</div>}
                                </div>
                                {adminEmail !== data.email && <button disabled={adminEmail === data.email} title="Double Click To Delete An Admin Account" onDoubleClick={() => handleDeleteAdminAccount(data._id)}><i className="fa fa-trash"></i></button>}
                            </li>
                        )
                    })
                }
