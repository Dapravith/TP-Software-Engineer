...
import "../styles/create-branch-account/create-branch-account.css";
import axios from "axios";
export default function CreateBranchAccount(props) {
    ...
    const handleCreateBranchAccount = (e) => {
        e.preventDefault();
        ...
        try {
            ...
            if (selectedFile !== null) {
                axios.post(`${process.env.REACT_APP_BACKEND}/branch-owner/register`, formData, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`
                    }
                }).then(data => {
                    if (data.data.message === "Branch Owner Account Created Successfully!") {
                        setCreated(true);
                        setTab(1);
                    }
                }).catch(err => {
    ...
    return (
        <div className="create-branch-account">
            ...
            {created && <RightTopAlert content={"Branch Owner Account Created Successfully!"} type="SUCCESS" />}
            <div className="create-branch-account-title">
                <h1>Create Branch Account</h1>
            </div>
            <form onSubmit={handleCreateBranchAccount}>
                <div>
                ...
                <div>
                    ...
                    <textarea placeholder="Describe about branch owner..." value={description} onChange={e => setDescription(e.target.value)}></textarea>
                </div>
                <div>
                ...
                <div>
                    <button type="submit" id="create">Create A Branch Account</button>
                </div>
            </form>
        </div>
    )
